    ...
    out.delimited_block(
        ...
        |out| {
            for field in &table.columns {
                ...
                writeln!(out, "//{:?}", &field.col_type).unwrap();
...
pub fn autogen_rust_globals(items: &[GenItem]) -> Vec<Vec<(String, String)>> {
    let mut output = CodeIndenter::new(String::new());
    let out = &mut output;
    ...
    print_handle_table_update_defn(out, items);
...
fn iter_module_names(items: &[GenItem]) -> impl Iterator<Item = String> + '_ {
    items.iter().filter_map(|item| match item {
        GenItem::Table(table) => Some(table.schema.table_name.to_case(Case::Snake)),
...
fn print_handle_table_update_defn(out: &mut Indenter, items: &[GenItem]) {
